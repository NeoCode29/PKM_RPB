...
interface KriteriaSubstansiTableProps {
  kriteria: KriteriaSubstansi[];
  onEdit: (kriteria: KriteriaSubstansi) => void;
  onDelete: (kriteria: KriteriaSubstansi) => void;
}

export const KriteriaSubstansiTable: React.FC<KriteriaSubstansiTableProps> = ({
  kriteria,
  onEdit,
  onDelete,
}) => {
      ...
      <Table>
        <TableHeader>
          <TableRow>
            ...
            <TableHead className="w-[120px]">Dibuat Pada</TableHead>
          ...
          {kriteria.length === 0 ? (
            <TableRow>
              <TableCell colSpan={5} className="text-center h-24">
                Belum ada data kriteria substansi
              </TableCell>
            </TableRow>
          ) : (
            kriteria.map((item) => (
